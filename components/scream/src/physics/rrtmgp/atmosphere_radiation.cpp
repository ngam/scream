...
int RRTMGPRadiation::requested_buffer_size_in_bytes() const
{
  const int interface_request = Buffer::num_1d_ncol*m_ncol*sizeof(Real) +
                                ...
                                Buffer::num_3d_nswbands*m_ncol*m_nlay*m_nswbands*sizeof(Real) +
                                Buffer::num_3d_nlwbands*m_ncol*m_nlay*m_nlwbands*sizeof(Real);
...
void RRTMGPRadiation::run_impl (const Real dt) {
  ...
  // Copy data from the FieldManager to the YAKL arrays
  {
    ...
    Kokkos::parallel_for(policy, KOKKOS_LAMBDA(const MemberType& team) {
      ...
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team, m_nswbands*m_nlay), [&] (const int&idx) {
          auto b = idx / m_nlay;
          auto k = idx % m_nlay;
      ...
      Kokkos::parallel_for(Kokkos::TeamThreadRange(team, m_nlwbands*m_nlay), [&] (const int&idx) {
          auto b = idx / m_nlay;
          auto k = idx % m_nlay;
